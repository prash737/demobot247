...
export default function DashboardPage() {
  ...
  const [unansweredQueries, setUnansweredQueries] = useState<
    { query: string; frequency: number }[]
  >([]);
  const [top10Queries, setTop10Queries] = useState<
    { query: string; frequency: number }[]
  >([]);
  ...
  const fetchData = async (period: string) => {
    ...
    try {
      ...
      for (let i = 0; i < 30; i++) {
        ...
        const { data: checkInsights, error: checkInsightsError } =
          await supabase
            .from("insights")
            .select("*")
            .eq("chatbot_id", chatbotId)
      ...
      const { data: yesterdayInsights, error: yesterdayInsightsError } =
        await supabase
          .from("insights")
          .select("*")
          .eq("chatbot_id", chatbotId)
      ...
      setInsightsData({
        ...
        yesterday_total_conversations:
          yesterdayInsights?.total_conversations || 0,
        yesterday_total_user_queries:
          yesterdayInsights?.total_user_queries || 0,
        yesterday_total_assistant_responses:
          yesterdayInsights?.total_assistant_responses || 0,
      ...
      if (foundInsights.unanswered_queries) {
        try {
          ...
          if (unansweredData && unansweredData.queries) {
            const queriesArray = Object.entries(unansweredData.queries).map(
              ([query, frequency]) => ({
                query,
            ...
            setTotalUnansweredCount(
              unansweredData.total_count || queriesArray.length,
            );
          } else {
            setUnansweredQueries([]);
            setTotalUnansweredCount(0);
      ...
      if (foundInsights.top_user_queries) {
        try {
          ...
          if (top10Data && top10Data.queries) {
            const queriesArray = Object.entries(top10Data.queries).map(
              ([query, frequency]) => ({
                query,
                frequency: frequency as number,
              }),
      ...
      const { data: plotFiles, error: plotsError } = await supabase.storage
        .from("plots")
        .list(
          `${chatbotId}/${foundDate.toISOString().split("T")[0]}/${period}`,
        );
        ...
        const plotUrls = plotFiles.map(
          (file) =>
            supabase.storage
              .from("plots")
              .getPublicUrl(
                `${chatbotId}/${foundDate.toISOString().split("T")[0]}/${period}/${file.name}`,
              ).data.publicUrl,
        );
        setPlots(plotUrls);
      }
        ...
        <div className="flex-1 overflow-auto">
          <div className="container mx-auto px-4 py-6">
            ...
            >
              <div className="mb-4 flex justify-between items-center">
                <h2
                  ...
                  style={{ color: INSIGHT_COLORS.heading }}
                >
                  Dashboard
                </h2>
                <Button
                ...
                {!loading && !error && insightsData && (
                  <>
                    ...
                    <InsightCard
                      ...
                      previousValue={
                        insightsData.yesterday_total_assistant_responses
                      }
                    ...
                    {(() => {
                      ...
                      if (userPlan === "Free Plan" || userPlan === "Basic") {
                        percentageUsed = Math.min(
                          Math.round((totalChats / limit) * 100),
                          100,
                        );
                      } else {
                        ...
                        percentageUsed = Math.min(
                          Math.round((totalChats / limit) * 100),
                          100,
                        ); // Static value for visual indication only
                      }
                          ...
                          <div className="flex justify-between items-center">
                            {isUnlimitedPlan ? (
                              ...
                              <span className="text-sm font-medium">
                                {totalChats} total conversations this month
                              </span>
                            ) : (
            ...
            {!loading &&
              !error &&
              (!insightsData ||
                (!conversations.length && plots.length === 0)) && (
                ...
                >
                  <div className="grid gap-6 sm:grid-cols-2">
                    {plots.map((plot, index) => {
                      const plotName = decodeURIComponent(
                        plot.split("/").pop()?.split(".")[0] ||
                          `Plot ${index + 1}`,
                      );
                      return (
                        <Card
                          key={index}
                          className="overflow-hidden flex flex-col h-[300px] sm:h-[400px] lg:h-[500px]"
                        >
